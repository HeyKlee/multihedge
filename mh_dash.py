...
// Touch support: toggle chat panel on tap for mobile devices where
// dblclick events do not fire naturally.
document.getElementById('xoraPet')?.addEventListener('touchend',function(e){
  const pet=document.getElementById('xoraPet');
  const wasDragging=pet&&pet.classList.contains('pet-dragging');
  if(!wasDragging){
    const panel=document.getElementById('chatPanel');
    panel.classList.toggle('open');
    if(panel.classList.contains('open')){
